...
def depth2norm(depth_tensor, bg_th=0.1, a=np.pi * 2.0):
    ...
    for i in range(batch_size):
        depth_np = depth_tensor[i,0].float().cpu().numpy()
...
def get_cond_model(opt, cond_type: ExtraCondition):
    if cond_type == ExtraCondition.sketch:
        ...
        return model
    elif cond_type == ExtraCondition.dex:
        from ldm.modules.midas.api import MiDaSInference
        midas = MiDaSInference(model_type="dpt_hybrid").to(opt.device)
        return midas
...
def get_cond_dex(opt, cond_image, cond_seg = None, cond_model=None, cond_mask = None):
    cc = cond_model(cond_image.to(opt.device))
    ...
    if cond_mask is not None:
        normals = normals * cond_mask.to(opt.device)
    
    return normals


def get_adapter_feature(input_skeleton, input_depth, mano, adapters):
    ret_feat_map = None
    ret_feat_seq = None
    inputs = [(input_skeleton, input_depth, mano)]
    adapters = [adapters]
