...
def diffusion_inference(opt, model, sampler, adapter_features, append_to_context=None, keypoint_feat=None):
    # get text embedding
    if not isinstance(opt.prompt, list):
        opt.prompt = [opt.prompt]
    c = model.module.get_learned_conditioning(opt.prompt)
    if opt.scale != 1.0:
        uc = model.module.get_learned_conditioning([opt.neg_prompt]*len(opt.prompt))
    else:
        uc = None
    ...
    x_samples = model.module.decode_first_stage(samples_latents)
